...
function _emailamender_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'uk.org.futurefirst.networks.emailamender';
      }
      $entities[] = $e;
    }
  }
...
function _emailamender_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_emailamender_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
    ...
    $caseTypes[$name] = array(
      'module' => 'uk.org.futurefirst.networks.emailamender',
      'name' => $name,
      'file' => $file,
    );
  }
 ...
 */
function _emailamender_civix_insert_navigation_menu(&$menu, $path, $item, $parentId = NULL) {
  static $navId;
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _emailamender_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
